...
test.describe('Create: As a user, I\'d like to add a new book to the shelf, so that I can update the list with my own recommendations.', () => {
  ...
  test.fixme('Create a BookFormModal component that contains the form elements required to collect the user input needed for creating a new book. Reveal this modal when the "Add Book" button is clicked, and hide the modal when the modal is closed.', async ({ page }) => {
...
test.describe('Delete: As a user, I want to remove books from my list, so that only the most important books are on my shelf.', () => {
  ...
  test.fixme('In your front-end Book component, add a "Delete" button component for every book in the list.', async ({ page }) => {
    await page.goto(REACT_APP_URL);
  ...
  test.fixme('When the user clicks the delete button, the book should be removed.', async ({ page }) => {
